...
class Orders {
private:
    static inline std::vector<std::shared_ptr<Menu>> orderedMenuList;
public:
    ...
    static void placeOrder(std::uint16_t menuId) {
        ...
        if (menuIt != Menu::menuProductList.end()) {
            orderedMenuList.push_back(*menuIt);
    ...
    static void cancelOrder(std::uint16_t menuId) {
        auto orderIt = std::find_if(orderedMenuList.begin(), orderedMenuList.end(), [menuId](const std::shared_ptr<Menu>& menu) {
            return menu->id == menuId;
        });
